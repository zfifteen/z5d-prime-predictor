...
def run_z5d_predictor(
    k: int, z5d_cli_path: Path, use_mock: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        result = subprocess.run(
            [str(z5d_cli_path), str(k)], capture_output=True, text=True, timeout=10
        )
...
def assign_bins(
    ...
    primes = np.array(
        [float(r["predicted_prime"]) for r in valid_results], dtype=np.float64
    )
...
def main():
    ...
    if not z5d_cli_path.is_absolute():
        z5d_cli_path = Path(__file__).resolve().parent.parent.parent / z5d_cli_path
    ...
    # Map to k values
    print(
        f"Mapping QMC samples to k values in range [10^{args.scale_min}, 10^{args.scale_max}]...",
        file=sys.stderr,
    )
    ...
    print(
        f"Keeping top {len(top_results)} results out of {len(valid_results)} valid predictions",
        file=sys.stderr,
    )
