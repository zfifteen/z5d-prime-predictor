...
def map_qmc_to_n(qmc_values: np.ndarray, scale_min: int, scale_max: int) -> np.ndarray:
    ...
    n_values = np.power(10.0, log_n).astype(object)  # Use object for large integers
...
def compute_geometric_resonance(N: int, k_or_phase: float, window_size: int = 1000) -> Tuple[float, int]:
    """
...
def extract_geofac_peaks(row_ids: List[int], qmc_samples: np.ndarray, 
    ...
    total = len(row_ids)
    for idx, (row_id, n_base, qmc_row) in enumerate(zip(row_ids, n_values, qmc_samples)):
        if idx % 1000 == 0:
...
def main():
    ...
    results = extract_geofac_peaks(row_ids, samples, args.scale_min, args.scale_max, args.max_samples)
    ...
    print(f"Keeping top {len(top_results)} results out of {len(valid_results)} valid results", file=sys.stderr)
